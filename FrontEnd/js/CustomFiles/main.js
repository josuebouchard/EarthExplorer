define(["require", "exports", "helpers", "three", "input"], function (require, exports, helpers_1, THREE, input_1) {
    "use strict";
    ...
    function logic() {
        //Handle rotation
        if (input_1.InputState.isDragging) {
            let displacement = input_1.InputState.displacement.get();
            displacementCamera.x += displacement.x / 500;
            displacementCamera.y -= displacement.y / 500;
        }
        //Handle zoom
        if (input_1.InputState.isScrolling) {
            let scroll = input_1.InputState.scrolling.get();
            cameraRadius += scroll.y / 1000;
        }
        //Putting limits
        displacementCamera.y = helpers_1.clamp(displacementCamera.y, 0.000001, Math.PI);
        cameraRadius = helpers_1.clamp(cameraRadius, 1.2, 4);
